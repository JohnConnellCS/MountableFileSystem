...
void write_block_bitmap(int fd)
{
	...
	// Set all bits to in use
	memset(map_value, 0xFF, BLOCK_SIZE);
	// Set bits 24-1023 to available
	map_value[2] = 0b01111111;
	memset(map_value + 3, 0x00, BLOCK_SIZE / 8 - 3);
...
void write_inode_bitmap(int fd)
{
	...
	// Set 13 bits to in use
	map_value[0] = 0xFF;
	map_value[1] = 0b00011111; 
	// set all non-free bits to 1(1024 / 8 = 128, start with 129th entry)
...
void write_inode_table(int fd) {
	...
	struct ext2_inode rootInode = {0};
	rootInode.i_mode = EXT2_S_IFDIR
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IXUSR
	                              | EXT2_S_IRGRP
	                              ...
	                              | EXT2_S_IXOTH;
	rootInode.i_uid = 0;
	rootInode.i_size = 1024;
	rootInode.i_atime = current_time;
	rootInode.i_ctime = current_time;
	rootInode.i_mtime = current_time;
	rootInode.i_dtime = 0;
	rootInode.i_gid = 0;
	rootInode.i_links_count = 3;
	rootInode.i_blocks = 2; 
	rootInode.i_block[0] = ROOT_DIR_BLOCKNO;
	write_inode(fd, EXT2_ROOT_INO, &rootInode);

	//Hello-World File
	struct ext2_inode helloWorldInode = {0};
	helloWorldInode.i_mode = EXT2_S_IFREG
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	helloWorldInode.i_uid = 1000;
	helloWorldInode.i_size = 12;
	helloWorldInode.i_atime = current_time;
	helloWorldInode.i_ctime = current_time;
	helloWorldInode.i_mtime = current_time;
	helloWorldInode.i_dtime = 0;
	helloWorldInode.i_gid = 1000;
	helloWorldInode.i_links_count = 1;
	helloWorldInode.i_blocks = 2; 
	helloWorldInode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	write_inode(fd, HELLO_WORLD_INO, &helloWorldInode);

	//Hello Symbolic Link Inode
	struct ext2_inode helloInode = {0};
	helloInode.i_mode = EXT2_S_IFLNK
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	helloInode.i_uid = 1000;
	helloInode.i_size = 11;
	helloInode.i_atime = current_time;
	helloInode.i_ctime = current_time;
	helloInode.i_mtime = current_time;
	helloInode.i_dtime = 0;
	helloInode.i_gid = 1000;
	helloInode.i_links_count = 1;
	helloInode.i_blocks = 0; 
	memcpy(helloInode.i_block, "hello-world", helloInode.i_size);
